...
import type { Hero, CreatureData } from '../types.js';

export interface ExchangeResult {
  heroDmg: number;     // dealt to creature
  creatureDmg: number; // dealt to hero
  creatureDead: boolean;
  heroDowned: boolean;
  log: string;
}

export function heroAttack(): number { return WEAPON_ATK[0]; }

export function exchange(hero: Hero, creature: CreatureData): ExchangeResult {
  ...
  if (hero.vigor > 0) hero.vigor = Math.max(0, hero.vigor - FIGHT_VIGOR);
...
export function flee(hero: Hero, creature: CreatureData): number {
  const arm = ARMOR_DEF[hero.gear.armor] ?? 0;
