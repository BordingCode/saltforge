...
export function initJuice(app: HTMLElement, fx: HTMLElement): void { appEl = app; fxEl = fx; }

export function addTrauma(amount: number): void { if (!reduceMotion) trauma = Math.min(1, trauma + amount); }

// Called once per frame from the render loop.
export function tickShake(): void {
  if (!appEl) return;
  if (trauma <= 0.001) { if (appEl.style.transform) appEl.style.transform = ''; trauma = 0; return; }
...
export function flash(color: string, dur = 320): void {
  if (reduceMotion || !fxEl) return;
...
export interface BurstOpts { count?: number; color?: string; spread?: number; size?: number; }
...
export function popText(x: number, y: number, text: string, color = '#fff'): void {
  if (!fxEl) return;
...
export function centerOf(sel: string): { x: number; y: number } | null {
...
export function pop(sel: string): void {
  if (reduceMotion) return;
