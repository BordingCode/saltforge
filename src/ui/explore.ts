...
export function renderExploreHUD(hud: HTMLElement, h: Handlers): void {
  clear(hud);
  const run = Game.run!; const world = Game.world!;
  const top = el('div', { class: 'hud-top' }, [topBar(run)]);
  const goal = currentStage();
  if (goal) top.appendChild(objectiveRibbon(goal, h));
  hud.appendChild(top);
  hud.appendChild(controls(run, world, h));
}

function objectiveRibbon(goal: { n: number; total: number; text: string }, h: Handlers): HTMLElement {
...
function topBar(run: any): HTMLElement {
  const res = (Object.keys(RES_META) as ResourceKind[]).map((k) =>
    el('div', { class: 'res' }, [
...
function menaceBar(val: number, armed: boolean): HTMLElement {
  return el('div', { class: 'stat' }, [
...
function controls(run: any, world: any, h: Handlers): HTMLElement {
  ...
  if (here.node) action = btn(`Harvest ${RES_META[here.node.kind].name}`, 'act-harvest', () => h.harvest());
  else if (here.loot) action = btn('Open cache', 'act-loot', () => h.grab());
  else action = btn('—', 'act-none disabled', () => {});
...
function padBtn(label: string, onclick: () => void): HTMLElement {
