...
export function unlockAudio(): void { const c = ensure(); if (c && c.state === 'suspended') c.resume(); }
export function setMuted(m: boolean): void { muted = m; if (master) master.gain.value = m ? 0 : 0.5; }
export function isMuted(): boolean { return muted; }

function tone(freq: number, dur: number, type: OscillatorType = 'sine', gain = 0.3, when = 0): void {
...
function noise(dur: number, cutoff: number, gain = 0.25, when = 0): void {
  const c = ensure(); if (!c || !master || muted) return;
  const t = c.currentTime + when;
