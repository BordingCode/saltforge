...
import { centerOn, worldToScreenX, worldToScreenY } from '../engine/canvas.js';
...
export function drawOverworld(view) {
    ...
    for (const [dc, dr] of adj) {
        ...
        ctx.strokeStyle = 'rgba(127,224,210,0.5)';
        ctx.lineWidth = 2;
        roundRect(ctx, x + 3, y + 3, ts - 6, ts - 6, 7);
        ctx.stroke();
    }
    drawHero(ctx, worldToScreenX(view, hx), worldToScreenY(view, hy), ts);
}
function drawFog(ctx, x, y, ts) {
    ctx.fillStyle = '#0c1418';
    ctx.fillRect(x, y, ts, ts);
    ctx.fillStyle = 'rgba(255,255,255,0.015)';
    ctx.fillRect(x + 1, y + 1, ts - 2, ts - 2);
}
function drawTile(ctx, t, x, y, ts) {
    ...
    if (t.terrain === 'base')
        fill = '#3a4d57';
    ctx.fillStyle = fill;
    roundRect(ctx, x + 1, y + 1, ts - 2, ts - 2, 6);
    ctx.fill();
    if (t.terrain === 'rock') {
        drawRock(ctx, x, y, ts);
        return;
    ...
    if (t.node)
        drawNode(ctx, t.node.kind, x, y, ts);
    else if (t.creature)
        drawCreature(ctx, t.creature, x, y, ts);
    else if (t.loot)
        drawLoot(ctx, x, y, ts);
}
function drawRock(ctx, x, y, ts) {
...
function drawNode(ctx, kind, x, y, ts) {
    ...
    else if (kind === 'firesalt') {
        star(ctx, cx, cy, r * 1.3, r * 0.5, 5);
        ctx.fillStyle = meta.color;
        ctx.fill();
    }
    else {
...
function drawLoot(ctx, x, y, ts) {
    const cx = x + ts / 2, cy = y + ts / 2, r = ts * 0.18;
    ctx.fillStyle = '#caa86a';
    roundRect(ctx, cx - r, cy - r * 0.7, r * 2, r * 1.4, 3);
    ctx.fill();
    ctx.fillStyle = '#7fe0d2';
    ctx.fillRect(cx - r, cy - 1, r * 2, 2);
}
function drawCreature(ctx, cr, x, y, ts) {
    ...
    if (shape === 'wisp') {
        ...
        ctx.fill();
        ctx.fillRect(cx - r * 1.1, cy - 2, r * 0.5, 4);
        ctx.fillRect(cx + r * 0.6, cy - 2, r * 0.5, 4);
    }
    else if (shape === 'maw') {
        ctx.beginPath();
        ...
        ctx.fillRect(cx - r * 0.5, cy - r * 0.3, r * 0.3, r * 0.3);
        ctx.fillRect(cx + r * 0.2, cy - r * 0.3, r * 0.3, r * 0.3);
    }
    // health pip
    ctx.fillStyle = 'rgba(0,0,0,0.4)';
    ctx.fillRect(cx - r, y + ts - 6, r * 2, 3);
    ctx.fillStyle = '#ff6b6b';
    ctx.fillRect(cx - r, y + ts - 6, r * 2 * (cr.hp / cr.maxHp), 3);
}
function drawHero(ctx, x, y, ts) {
    const cx = x + ts / 2, cy = y + ts / 2;
    ctx.fillStyle = 'rgba(0,0,0,0.25)';
    ctx.beginPath();
    ctx.ellipse(cx, cy + ts * 0.28, ts * 0.22, ts * 0.08, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#f4e9d0';
    ctx.beginPath();
    ctx.arc(cx, cy - ts * 0.16, ts * 0.12, 0, Math.PI * 2);
    ctx.fill(); // head
    ctx.fillStyle = '#2f6f78';
    roundRect(ctx, cx - ts * 0.14, cy - ts * 0.04, ts * 0.28, ts * 0.3, 4);
    ctx.fill(); // body
    ctx.strokeStyle = '#e8b84b';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(cx + ts * 0.16, cy - ts * 0.1);
    ctx.lineTo(cx + ts * 0.16, cy + ts * 0.2);
    ctx.stroke(); // tool
}
