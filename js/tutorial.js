...
let meta = (() => {
    try {
        const m = JSON.parse(localStorage.getItem(TKEY) || '{}');
        return { introSeen: !!m.introSeen, done: !!m.done };
    }
    catch {
        return { introSeen: false, done: false };
    }
})();
function persist() { try {
    localStorage.setItem(TKEY, JSON.stringify(meta));
}
catch { /* ignore */ } }
export function tutorialActive() { return !meta.done; }
export function shouldShowIntro() { return !meta.introSeen; }
export function markIntroSeen() { if (!meta.introSeen) {
    meta.introSeen = true;
    persist();
} }
export function completeTutorial() { if (!meta.done) {
    meta.done = true;
    persist();
} }
const STAGES = [
...
export function currentStage() {
    if (meta.done || !Game.run)
        return null;
    for (let i = 0; i < STAGES.length; i++)
        if (!STAGES[i].done())
            return { n: i + 1, total: STAGES.length, text: STAGES[i].text };
    return null; // every stage cleared
}
export function allStagesDone() { return !!Game.run && STAGES.every((s) => s.done()); }
