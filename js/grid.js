...
export const cellKey = (cell) => key(cell.col, cell.row);
export function inBounds(c, r, w, h) {
    return c >= 0 && r >= 0 && c < w && r < h;
}
// 4-neighbour (orthogonal) — movement is orthogonal, tap an adjacent tile.
export function neighbours4(c, r, w, h) {
    const out = [];
    if (r > 0)
        out.push({ col: c, row: r - 1 });
    if (r < h - 1)
        out.push({ col: c, row: r + 1 });
    if (c > 0)
        out.push({ col: c - 1, row: r });
    if (c < w - 1)
        out.push({ col: c + 1, row: r });
    return out;
}
// 8-neighbour (for fog reveal radius / blast area).
export function neighbours8(c, r, w, h) {
    const out = [];
...
export const euclid = (a, b) => Math.hypot(a.col - b.col, a.row - b.row);
// Cells within Chebyshev radius r of (c,row) — fog reveal / lantern radius.
export function within(c, row, radius, w, h) {
    const out = [];
    for (let dr = -radius; dr <= radius; dr++)
        for (let dc = -radius; dc <= radius; dc++) {
            const nc = c + dc, nr = row + dr;
            if (inBounds(nc, nr, w, h) && Math.max(Math.abs(dc), Math.abs(dr)) <= radius)
                out.push({ col: nc, row: nr });
        }
    return out;
}
// BFS shortest path length over passable cells (return-to-base distance). Returns -1 if unreachable.
export function bfsDistance(start, goal, passable, w, h) {
    ...
    while (frontier.length) {
        ...
        for (const cell of frontier) {
            for (const n of neighbours4(cell.col, cell.row, w, h)) {
                const k = cellKey(n);
                if (seen.has(k) || !passable(n.col, n.row))
                    continue;
