...
export function renderBattleship(overlay, bs, h) {
    ...
    // enemy cell lookup
    const occ = new Set(enemy.structures.flatMap((s) => s.cells));
    const sunkCells = new Set(enemy.structures.filter(isSunk).flatMap((s) => s.cells));
    ...
    for (let r = 0; r < BS_GRID; r++)
        for (let c = 0; c < BS_GRID; c++) {
            ...
            let cls = 'bs-cell';
            if (sunkCells.has(cell))
                cls += ' sunk';
            else if (hit)
                cls += ' hit';
            else if (shot)
                cls += ' miss';
            else if (enemy.knownShip.has(cell))
                cls += ' know-ship';
            else if (enemy.knownEmpty.has(cell))
                cls += ' know-empty';
...
function onCell(c, r, bs, h) {
    ...
    if (bs.shotsLeft <= 0) {
        bs.message = 'No shots left — load a salvo.';
        return;
    }
    h.fireSalvo(c, r);
}
function scanBtn(label, kind, avail, cost, bs, h) {
    const ammo = Game.run.resources.firesalt;
    const costN = cost.firesalt ?? 0;
    const ok = avail && !bs.scanUsed && ammo >= costN;
    return el('button', { class: `scan ${ok ? '' : 'disabled'}`, onclick: () => { if (ok)
            h.scan(kind); } }, [
