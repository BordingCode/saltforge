...
import { BUILDINGS, GEAR_SLOTS, GEAR_META, GEAR_TIER_NAMES, RES_META, BUILDING_BY_ID, WEAPON_ATK, ARMOR_DEF, LANTERN_RADIUS, SALTERN_INCOME, CANNON_SHOTS, KEEP_GRID_HP, MAX_GEAR_TIER, } from '../config.js';
export function renderBase(overlay, h) {
    const run = Game.run;
...
function resStrip(run) {
    return el('div', { class: 'res-strip' }, Object.keys(RES_META).map((k) => el('div', { class: 'res' }, [
...
function costStr(cost) {
    if (!cost)
        return '';
    return Object.keys(cost).map((k) => `${cost[k]} ${RES_META[k].glyph}`).join('  ');
}
function buildingEffect(id, lvl) {
    switch (id) {
...
function buildingRow(id, h) {
    ...
    const reqText = def.requires ? 'Needs ' + Object.entries(def.requires).map(([k, v]) => `${BUILDING_BY_ID[k].name} ${v}`).join(', ') : '';
    return el('div', { class: `card ${locked ? 'locked' : ''}` }, [
        ...
        el('div', { class: 'card-buy' }, [
            ...
            el('button', {
                ...
                onclick: () => { if (!maxed && req && afford)
                    h.upgrade(id); },
            }, [btnLabel]),
        ]),
    ]);
}
function gearEffect(slot, tier) {
    if (slot === 'weapon')
        return `Attack ${WEAPON_ATK[tier]}`;
    if (slot === 'armor')
        return `Block ${ARMOR_DEF[tier]} / hit`;
    return `See ${LANTERN_RADIUS[tier]} tiles, faster harvest`;
}
function gearRow(slot, h) {
    const run = Game.run;
    ...
    return el('div', { class: `card ${hasForge ? '' : 'locked'}` }, [
        ...
        el('div', { class: 'card-buy' }, [
            ...
            el('button', {
                ...
                onclick: () => { if (!maxed && hasForge && afford)
                    h.craft(slot); },
