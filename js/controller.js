...
export const handlers = {
    ...
    harvest() {
        ...
        if (got) {
            ...
            if (p)
                popText(p.x, p.y - 8, `+${got.amount}`, RES_META[got.kind].color);
            addTrauma(0.05);
        }
        afterAction();
    },
    grab() {
        ...
        if (got) {
            ...
            if (p)
                popText(p.x, p.y - 8, `+${got.amount} ${RES_META[got.kind].name}`, RES_META[got.kind].color);
            addTrauma(0.08);
        }
        afterAction();
    },
...
function doAttack() {
    ...
    if (foePos) {
        burst(foePos.x, foePos.y, { color: '#ff8a4a', count: ex.creatureDead ? 22 : 11, spread: ex.creatureDead ? 88 : 52, size: 7 });
        popText(foePos.x, foePos.y, `-${ex.heroDmg}`, '#ffd54a');
    }
    pop('.foe-name');
    ...
    if (ex.creatureDmg > 0) {
        ...
        if (mePos)
            popText(mePos.x, mePos.y, `-${ex.creatureDmg}`, '#ff6b6b');
    }
    if (ex.heroDowned) {
        encounter = null;
        overlay = 'none';
...
function doFire(c, r) {
    ...
    if (res.hit) {
        ...
        if (pos)
            burst(pos.x, pos.y, { color: '#4a7c8a', count: 6, spread: 28, size: 5 });
    }
    if (res.won) {
        winRun();
        sfx.win();
        flash('rgba(232,184,75,0.3)');
        addTrauma(0.8);
        haptic([0, 60, 50, 60, 50, 90]);
        flush();
        toEnd();
        saveRun();
        return;
