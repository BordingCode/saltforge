...
import { chebyshev, inBounds, neighbours4 } from '../grid.js';
import { WORLD_W, WORLD_H, BASE_POS, RIVAL_POS, bandFor } from '../config.js';
import { CREATURES_BY_BAND, makeCreature } from '../data/creatures.js';
export const idx = (w, c, r) => r * w.w + c;
export const tileAt = (w, c, r) => w.tiles[r * w.w + c];
function distToBase(c, r) {
...
export function generateWorld(seed) {
    ...
    for (const n of neighbours4(BASE_POS.col, BASE_POS.row, w, h))
        tiles[n.row * w + n.col].terrain = 'ground';
    const world = { w, h, tiles, base: { ...BASE_POS }, rival: { ...RIVAL_POS } };
    ...
    for (let r = 0; r < h; r++) {
        for (let c = 0; c < w; c++) {
            ...
            if (t.terrain !== 'ground')
                continue;
            if (c === BASE_POS.col && r === BASE_POS.row)
                continue;
            const dist = distToBase(c, r);
            ...
            if (!nearBase && rng.chance(band.nodeChance)) {
                const kind = rng.pick(band.yields);
    ...
    for (const cell of corridor) {
        if (fsPlaced >= 4)
            break;
        if (cell.col === RIVAL_POS.col && cell.row === RIVAL_POS.row)
            continue;
        if (bandFor(distToBase(cell.col, cell.row)).id !== 4)
            continue;
...
function carveCorridor(world, a, b) {
    ...
    while ((c !== b.col || r !== b.row) && guard++ < world.w * world.h) {
        if (Math.abs(b.col - c) >= Math.abs(b.row - r) && c !== b.col)
            c += stepC;
        else if (r !== b.row)
            r += stepR;
        else
            c += stepC;
        if (!inBounds(c, r, world.w, world.h))
            break;
...
export function passable(world, c, r) {
    if (!inBounds(c, r, world.w, world.h))
        return false;
